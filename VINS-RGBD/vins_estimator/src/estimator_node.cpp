...
std::vector<std::tuple<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr, sensor_msgs::PointCloud2ConstPtr>>
getMeasurements()
{
    std::vector<std::tuple<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr, sensor_msgs::PointCloud2ConstPtr>> measurements;

    while (true)
    {
        if (imu_buf.empty() || feature_buf.empty() || points_buf.empty())
            return measurements;
        ...
        sensor_msgs::PointCloudConstPtr img_msg = feature_buf.front();
        feature_buf.pop();
        sensor_msgs::PointCloud2ConstPtr point_msg = points_buf.front();
        points_buf.pop();
        ...
        if (IMUs.empty())
            ROS_WARN("no imu between two image");
        measurements.emplace_back(IMUs, img_msg, point_msg);
    }
    return measurements;
}
...
void process()
{
    while (true)
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            auto point_msg = std::get<2>(measurement);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_points = n.subscribe("/feature_tracker/points", 2000, points_callback);
